...
//		echo $this->db->last_query();
	...
	function get_ref_instansi_internal($key) {
		$sql = "SELECT os.organization_structure_id AS id, os.unit_name AS value, us.jabatan, u.user_name AS nama_pejabat, u.external_id AS nip_pejabat, dir.unit_name AS instansi, os.unit_code
	...
	function get_default_tujuan_sme() {
		$key = strtolower('DIREKTORAT UTAMA');
	...
	function get_ref_internal($organization_structure_id) {
		if ($organization_structure_id) {
			$sql = "SELECT os.organization_structure_id AS id, os.unit_name AS value, us.jabatan, u.user_name AS nama_pejabat, u.user_id, u.email, os.ske_sign, os.sub_id, os.abv, os.level, os.official_code, u.external_id AS nip_pejabat, dir.unit_name AS instansi, os.unit_code
					  FROM system_security.organization_structure os
	...
	function get_klasifikasi_arsip($code) {		
		$sql = "
	SELECT kss.entry_id, kss.kode_klasifikasi kode_klasifikasi_sub_sub, kss.nama_klasifikasi nama_klasifikasi_sub_sub, ks.nama_klasifikasi nama_klasifikasi_sub, k.nama_klasifikasi
...
//			}
	...
	function get_parent_function() {
		...
		$rows = $this->db->get_where('system_security.function_ref', array('module_ref_id' => 1));
