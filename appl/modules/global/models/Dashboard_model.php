...
class Dashboard_model extends LX_Model {
	...
	function get_my_surat($jenis_agenda = NULL) {
		$param = " s.jenis_agenda = '$jenis_agenda' AND s.status <> 99 ";
		if(!has_permission(7)) {
			$param = " s.jenis_agenda = '$jenis_agenda' AND osa.status IS NULL";
		}
	...
	function get_my_list($type) {
		...
		switch ($type) {
			case 'surat_masuk_eksternal' :
 				...
 				if(!has_permission(7)) {
 					$param = " s.jenis_agenda = 'SME' AND osa.status IS NULL";
 				}
 				...
 				$sql = "SELECT s.surat_id id, s.agenda_id, s.jenis_agenda, 
 							   ...
 							   COALESCE(fp.title, 'Arsip TU') status_surat,
 							   n.detail_link link,
 							   ('surat/external/incoming/' || s.surat_id) link1,
 								s.status
 						  FROM surat s
 						  JOIN notify n ON(n.ref_id = s.surat_id AND n.function_ref_id = s.function_ref_id " . ((!has_permission(7)) ? (" AND notify_user_id = '" . get_user_id() . "'") : '') . ")
 						  LEFT JOIN org_struc_archive osa ON(osa.ref_type = 'surat' AND osa.ref_id = s.surat_id " . ((!has_permission(8)) ? (" AND osa.organization_structure_id = " . get_user_data('unit_id')) : '' ) . ")
			...
			case 'surat_keluar_eksternal' :
				$param = " s.jenis_agenda = 'SKE' AND s.status <> 99 ";
 				...
 				$sql = "SELECT s.surat_id id, s.agenda_id, s.jenis_agenda, 
 							   ...
 							   COALESCE(fp.title, 'Arsip TU') status_surat,
 							   n.detail_link link,
 							   ('surat/external/outgoing/' || s.surat_id) link1,
 								s.status
 						  FROM surat s
			...
			case 'surat_internal' :
				...
				$sql = "SELECT s.surat_id id, s.agenda_id, s.jenis_agenda,
 							   ...
 							   COALESCE(fp.title, 'Arsip TU') status_surat,
 							   n.detail_link link,
 							   ('surat/internal/sheet/' || s.surat_id) link1,
 								s.status
 						  FROM surat s
			...
			case 'disposisi' :
 				$sql = "SELECT d.disposisi_id id, s.agenda_id, s.jenis_agenda, 
 							   ...
 							   '-' status_surat,
 							   n.detail_link link,
 							   ('surat/disposisi/sheet/' || d.disposisi_id) link1
 						  FROM disposisi d
 						  JOIN surat s ON(d.ref_type = 'surat' AND d.ref_id = s.surat_id)
 						  JOIN notify n ON(n.ref_id = d.disposisi_id AND n.agenda = 'disposisi' AND notify_user_id = '" . get_user_id() . "')
 						 WHERE d.status <> 99";
			...
			case 'tugas' :
				...
				if(!has_permission(7)) {
					$param = " s.jenis_agenda = 'ST' AND osa.status IS NULL ";
				}
				...
				$sql = "SELECT s.surat_id id, s.agenda_id, s.jenis_agenda,
 							   ...
 							   COALESCE(fp.title, 'Arsip TU') status_surat,
 							   n.detail_link link,
 							   ('surat/tugas/tugas/' || s.surat_id) link1,
 								s.status
 						  FROM surat s
