			</div><!-- /.content-wrapper -->
			<footer class="main-footer">
				<div class="pull-right hidden-xs">
					<b>Version</b> 1.0.0
				</div>
				<strong>Copyright &copy; 2017 Buana Varia Komputama.</strong> All rights reserved.
			</footer>
			<a href="#" class="gototop "><i class="fa fa-angle-up  fa-3x"></i></a>
		</div><!-- ./wrapper -->

		<!-- SlimScroll -->
		<script src="<?php echo assets_url(); ?>/plugins/slimScroll/jquery.slimscroll.min.js"></script>
		<!-- FastClick -->
		<script src="<?php echo assets_url(); ?>/plugins/fastclick/fastclick.min.js"></script>
		<!-- AdminLTE App -->
		<script src="<?php echo assets_url(); ?>/js/app.min.js"></script>
		<!-- AdminLTE for demo purposes -->
		<script src="<?php echo assets_url(); ?>/js/demo.js"></script>
		<script src="<?php echo assets_url(); ?>/plugins/pie-menu/js/jquery.menu.js"></script>
		<!-- script src="<?php echo assets_url(); ?>/js/demo1.js"></script-->
	</body>
</html>
