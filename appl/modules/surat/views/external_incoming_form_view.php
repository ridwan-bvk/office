...
<?php 
if(isset($surat->distribusi_tujuan)) {
...
?>
					<div class="checkbox <?php echo (isset($distribusi['direksi'][$row->organization_structure_id])) ? 'selected' : ''; ?>"><label><input type="checkbox" <?php echo (isset($distribusi['direksi'][$row->organization_structure_id]))? 'checked="checked"' : ''; ?> disabled="disabled"> <?php echo '( ' . $row->unit_code . ' ) ' . $row->unit_name; ?></label></div>
<?php
		}
?>
				</div>
...
?>
									 	<dt><?php echo $row->pengiriman_time ?></dt>
										<dd><?php //var_dump($list_ekspedisi) ?>Pengirim : <strong><?php echo $row->petugas_pengirim; ?></strong> / Penerima : <strong><?php echo $petugas ?></strong></dd>
										<dd>Status : <strong><?php echo $status_ekspedisi; ?></strong></dd>
								<!-- 	<dd>Pengantar : <a href="<?php echo site_url('surat/ekspedisi/sheet/' . $row->ekspedisi_id); ?>" target="_blank"><?php echo $row->ekspedisi_id; ?></a></dd> -->
										<br>
 <?php								
...
?>
			...
			<button type="submit" class="btn btn-app" onclick="location.assign('<?php echo site_url('surat/surat_model/init_arsip_sme/' . $surat->surat_id); ?>');">
...
?>
		<div class="box-body">
			<div class="form-group">
				...
				<div class="col-md-9">
					<input type="text" id="klasifikasi" class="form-control requiered" disabled="disabled" value="<?php echo $surat->kode_klasifikasi_arsip; ?>">
				</div>
			</div>
...
<?php 		
			if($surat->status > 5 && $surat->status != 99) {
?>
			<button type="submit" class="btn btn-app" onclick="location.assign('<?php echo site_url('surat/surat_model/init_arsip_sme/' . $surat->surat_id); ?>');">
				<i class="fa fa-archive"></i> Simpan Arsip
			</button>
			...
			if(result) {
				$.ajax({
					...
					function_ref_id: '<?php echo $function_ref_id; ?>',
							...
							surat_ext_nama:'<?php echo $surat_from_ref_data['nama']; ?>',
