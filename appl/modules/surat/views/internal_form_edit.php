			...
			<div class="form-group">
				...
				<div class="col-lg-3 col-sm-9">
					<div class="input-group">
						<input type="number" id="surat_item_lampiran" name="surat_item_lampiran" class="form-control" value="<?php echo $surat->surat_item_lampiran; ?>">
...
?>
					<div id="row_tembusan_<?php echo $i; ?>" class="form-group">
						<div class="col-sm-12">
							<div class="input-group">
								<input type="text" id="tembusan_<?php echo $i; ?>" name="tembusan[<?php echo $i; ?>]" class="form-control" data-input-title="Tembusan <?php echo $i; ?>" value="<?php echo $tembusan; ?>" placeholder="Tembusan surat...">
...
<?php 
		$i++;
		$param['tembusan'] .= $i . '. ' . $tembusan . '<br>';
	}
	
	if($param['tembusan'] != '') {
		$param['tembusan'] = 'Tembusan : <br>' . $param['tembusan'];
	}
?>
				</div><!-- /.box-body -->
			</div><!-- /.box -->
			...
			if(has_permission($process->permission_handle)) {
				if($process->modify == 1 || $process->button_return != '-' || $process->button_process != '-') {
					if($process->button_process != '-') {
						...
						$approved = TRUE;
						if($surat->status == 1 && $approval_status == 0) {
							$approved = FALSE;
						}
...
//	var optSrtJab = '<?php //echo str_replace("\n", '', form_dropdown('surat_eksternal_ttd[%0%][type_ttd]', $opt_ttd, (set_value('surat_int_penandatangan') ? set_value('surat_int_penandatangan') : 'Penandatangan' ), (' id="surat_int_penandatangan_%0%" class="form-control" ')) ); ?>';
	...
	function addTembusan() {
		...
		row = '<div id="row_tembusan_' + tembusanRow + '" class="form-group">' +
				'	<div class="col-sm-12">' +
				'		<div class="input-group">' +
				'				<input type="text" id="tembusan_' + tembusanRow + '" name="tembusan[' + tembusanRow + ']" class="form-control" data-input-title="Tembusan ' + tembusanRow + '" value="" placeholder="Tembusan surat...">' +
				'				<div class="input-group-btn">' +
