...
?>
	...
	<div class="box box-primary">
		<div class="box-header with-border">
			<span class="h3 box-title">Konsep Disposisi</span>
			<div class="box-tools pull-right">
				<button class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip" title="Collapse"><i class="fa fa-plus"></i></button>
			</div>
		</div>
	...
	if($surat->status != 4) {
		if(has_permission($process->permission_handle) && ($process->modify == 1  || $process->button_return != '-' || $process->button_process != '-')) {
				...
				if($process->button_process != '-') {
					...
					{
?>
					<button type="button" class="btn btn-app pull-right bg-green <?php echo ($surat->distribusi_tujuan == Null) ? 'hide' : ''; ?>" onclick="prosesData();">
