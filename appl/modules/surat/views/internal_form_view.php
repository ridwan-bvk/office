			...
			<div class="box box-primary collapsed-box">
				...
				<div class="box-body" style="display: none;">
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="text" id="klasifikasi" class="form-control" disabled="disabled" value="<?php echo (isset($surat->kode_klasifikasi_arsip)) ? $surat->kode_klasifikasi_arsip : ''; ?>">
						</div>
					</div>
					<div class="form-group">
...
?>
		...
		<div class="box-header with-border">
			<h3 class="box-title"> Konsep Surat Internal </h3>
			<div class="box-tools pull-right">
...
?>
				...
				<div class="col-xs-8">
<?php 		
		if(has_permission($process->permission_handle) || $process->permission_handle == 0) {	
			if($process->button_process != '-') {
				
				$approved = FALSE;
				
				if($surat->status == 1 && (isset($approval['direksi']) && $approval['direksi']['status'] == 1)) {
					$approved = TRUE;
				}
				...
				if(in_array($surat->status, array(2, 3, 4))) {
					$approved = TRUE;
				}
...
<?php 
			...
			if(($process->button_return) != '-' && ($surat->status < 6)) {
?>
					<button id="btn-return" type="button" class="btn btn-app pull-right bg-red" onclick="returnData();">
						<i class="fa fa-caret-square-o-left"></i> <?php echo $process->button_return; ?>
					</button>
<?php 
			...
			if(get_user_data('unit_id') == $surat->surat_to_ref_id && $surat->status >= 7) {
?>
					<button type="button" class="btn btn-app pull-right bg-green" onclick="prosesArsipUnit();">
						<i class="fa fa-caret-square-o-right"></i> Simpan Sebagai Arsip
					</button>
