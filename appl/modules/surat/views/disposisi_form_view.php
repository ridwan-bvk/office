...
<?php 
 	$this->load->view('disposisi_ref_' . $ref->jenis_agenda);
?>
	<!-- Default box -->
	<div class="row">
		<div class="col-md-6">
			<div id="distribusi" class="box box-primary">
		...
		<div class="col-md-6">
			...
			<div class="box box-primary">
				<div class="box-header with-border">
					<span class="h3 box-title">lampiran Disposisi </span> <span class="small">Max. 8MB (*.pdf, *.jpg, *.jpeg, *.png, *.zip, *.rar) </span>
					<div class="box-tools pull-right">
...
?>
				<button class="btn btn-box-tool" data-toggle="tooltip" title="Teruskan Disposisi" onclick="location.assign('<?php echo site_url('surat/disposisi/create_from/' . $disposisi->ref_type . '/' . $disposisi->ref_id . '/' . $disposisi->disposisi_id); ?>');"><i class="fa fa-share"></i></button>
<?php
		}
?>
			</div>
...
<?php 
		...
		$list = $this->disposisi_model->get_subordinates(get_user_data('unit_id'), 1, 2);
	//	var_dump($list);
...
<?php
				if($row->status == 0) {
?>
								<form id="dist_att_<?php echo $i; ?>" class="dist_att complete_state_off_<?php echo $i; ?>" action="<?php echo str_replace($this->config->item('url_suffix'), "", site_url('global/dashboard/ajax_handler')); ?>/" method="post" enctype="multipart/form-data" style="float: right;">
								...
								<script type="text/javascript">
									...
									$('#dist_att_<?php echo $i; ?>').ajaxForm({
										beforeSend: function(xhr) {
											d_<?php echo $i; ?>++;
											var bar = '<div id="distribusi_<?php echo $key; ?>_attachment_' + d_<?php echo $i; ?> + '" class="priv_attach"><div class="progress progress-xxs">' +
										...
										complete: function(xhr) {
											if(xhr.responseJSON.error != 0) {
												...
												$('#distribusi_<?php echo $key; ?>_attachment_' + d_<?php echo $i; ?>).html(newFile);
												pa[<?php echo $i; ?> - 1]++;
...
?>
								...
								<script type="text/javascript">
									...
									$('#dist_att_<?php echo $i; ?>').ajaxForm({
										beforeSend: function(xhr) {
											d_<?php echo $i; ?>++;
											var bar = '<div id="distribusi_<?php echo get_user_id(); ?>_attachment_' + d_<?php echo $i; ?> + '" class="priv_attach"><div class="progress progress-xxs">' +
														'	<div class="progress-bar progress-bar-danger progress-bar-striped" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 0%">' +
														'	</div>' +
														' </div></div>';
											$('#distribusi_<?php echo $i; ?>_attachment_list').append(bar);
										...
										uploadProgress: function(event, position, total, percentComplete) {
											var percentVal = percentComplete + '%';
											$('#distribusi_<?php echo get_user_id(); ?>_attachment_' + d_<?php echo $i; ?> + ' .progress-bar ').css({'width': percentVal});
											console.log('up > ' + percentVal);
										},
										success: function(xhr) {
...
?>
								<div id="distribusi_<?php echo $key; ?>_attachment_<?php echo $j; ?>" class="priv_attach" >
									...
									<a class="btn btn-xs btn-default" href="<?php echo $priv_attach->file; ?>" target="_blank" title="<?php echo $priv_attach->file_name; ?>"><i class="fa fa-file-text-o"></i> </a>
									<label id="distribusi_<?php echo get_user_id(); ?>_flabel_<?php echo $j; ?>"><?php echo $priv_attach->file_name; ?></label>
...
?>
	function addInstruksi() {
		...
		row = '<div id="instruksi_' + i + '" class="box box-primary">' +
				...
				'					<button type="button" class="btn btn-success pull-right" onclick="konfirmInstruksi(' + i + ')" title="Konfirmasi Instruksi..."><i class="fa fa-check"></i></button>' +
				'				</div>' +
				'			</div>' +
				'		</div>' +
				'	</div>' +
...
?>
	...
	function removePersonalAttachment(rid, key, fn) {
		if(typeof(key) == 'undefined') {
			$('#' + rid).remove();
		} else { 
			bootbox.confirm('Hapus lampiran ', function(result){
				if(result) {
					$.ajax({
						type: "POST",
						url: "<?php echo str_replace($this->config->item('url_suffix'), "", site_url('global/dashboard/ajax_handler')); ?>/",
						data: {action: 'surat.disposisi_model.remove_private_attachment', 
								ref_id: '<?php echo $disposisi->disposisi_id; ?>', 
								key: key,
								file: fn,
						},
						success: function(data){
							if(typeof(data.error) != 0) {
								$('#' + rid).remove();
							} else {
								bootbox.alert(data.message);
							}
