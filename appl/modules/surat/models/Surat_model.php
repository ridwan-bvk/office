...
class Surat_model extends LX_Model {
	...
	function get_all_parents($unit_id) {
		...
		$sql = "SELECT os.*, us.jabatan, u.user_id, u.external_id nip, u.user_name, u.email  FROM system_security.organization_structure os
				...
				WHERE os.organization_structure_id = (SELECT parent_id FROM system_security.organization_structure WHERE organization_structure_id = $unit_id) ORDER BY us.jabatan DESC";
...
//			}
	...
	function init_terima_si($surat) {
		...
		$this->db->update('ekspedisi', array('penerima_time' => date('Y-m-d H:i:s'), 'petugas_penerima' => get_user_data('user_name')), array('ekspedisi_id' => $surat->surat_pengantar_id));
...
// 		$this->output->set_content_type('application/json')->set_output(json_encode($return));
	...
	function get_current_eksternal_no() {
		...
		if($result->num_rows() > 0) {
			...
			$konsep_surat['konsep_text'] = str_replace(($surat->kode_klasifikasi_arsip . '/' . $surat->surat_from_ref . '/________/' . date('Y')), $return['surat_no'], $konsep_surat['konsep_text']);
			$konsep_surat['konsep_text'] = str_replace('__-__________-____', $return['surat_tgl'], $konsep_surat['konsep_text']);
...
//			$list_tujuan = user_in_unit(get_user_data('unit_id'));
	...
	function save_surat_keluar_send() {
		...
		redirect('surat/external/kirim_surat_keluar/' . $surat_id);
		exit;
