...
class Keputusan_model extends LX_Model {
	 ...
	 * 
	 */
	function insert_keputusan() {
// 		var_dump($_POST);
// 		var_dump($_FILES);
// 		exit;
		if($this->_validate_post_data($this->data_object->surat_keputusan, 'add') != FALSE) {
			
			$result = $this->db->get_where('system_security.function_ref', array('module_function' => 'keputusan/input_keputusan'));
			$function_data = $result->row();
					
			$keputusan_id = generate_unique_id();
			
			$data = array_intersect_key($_POST, $this->data_object->surat_keputusan);
			$data['surat_id'] = $keputusan_id; 
			...
			$data['surat_awal'] = human_to_db($data['surat_tgl_masuk']);
 			$data['surat_akhir'] = ($_POST['surat_unit_lampiran'] != NULL) ? human_to_db($data['surat_unit_lampiran']) : NULL;
			...
			if(isset($_POST['surat_tgl_masuk'])) {
 			...
 			if ($_POST['surat_unit_lampiran'] != NULL) {
 				$data['surat_unit_lampiran'] = human_to_db($data['surat_unit_lampiran']);
 			}
			...
			// set workspace
			$note = 'Surat Keputusan No. ' . $data['surat_no'] . ' tentang ' . $data['surat_perihal'];
			$this->db->insert('notify', array('function_ref_id' => $_POST['function_ref_id'], 'ref_id' => $data['surat_id'], 'agenda' => ('SK -' . $data['agenda_id']), 'note' => $note, 'detail_link' => ('surat/keputusan/keputusan_view/' . $data['surat_id']), 'notify_user_id' => get_user_id(), 'status' => 0, 'read' => 0));
				...
				foreach($_POST['attachment'] as $k => $v) {
					if($v['title'] != '' && !empty($_FILES['attachment_file_' . $k]['name'])) {
						if (!$this->upload->do_upload('attachment_file_' . $k)) {
							...
							$file_attached['ref_id'] = $keputusan_id;
	...
	function konfirmasi() {
 		...
 		$return = array('error' => '', 'message' => '', 'execute' => "location.reload()");
 			...
 			$this->db->update('surat', array('status' => 1), array('surat_id' => $_POST['ref_id']));
...
// 		exit;
		if($this->_validate_post_data($this->data_object->surat_keputusan, 'add') != FALSE) {
			...
			$data['surat_tgl_masuk'] = human_to_db($data['surat_tgl_masuk']);
			$data['surat_awal'] = human_to_db($data['surat_tgl_masuk']);
 			$data['surat_akhir'] = human_to_db($data['surat_unit_lampiran']);
...
//  		exit;
			...
			if(has_permission(23)) {
				...
				foreach ($list_tujuan->result() as $row) {
					...
					$this->db->insert('notify', array('function_ref_id' => $surat->function_ref_id, 'ref_id' => $_POST['surat_id'], 'notify_user_id' => $row->user_id));
...
// 		exit;
		if($this->_validate_post_data($this->data_object->surat_keputusan, 'add') != FALSE) {
			...
			$this->db->update('surat',array('status'=> 2, 'catatan_pengiriman' => $_POST['catatan_pengiriman'],'surat_ringkasan' => $_POST['surat_ringkasan']), array('surat_id' => $surat_id));
		...
		$note = 'Kontrak No. ' . $_POST['surat_no'] . ' Sudah Dihentikan ';
		$this->db->delete('notify', array('function_ref_id' => $_POST['function_ref_id'], 'ref_id' =>  $surat_id, 'notify_user_id' => get_user_id()));
		$this->db->insert('notify', array('function_ref_id' => $_POST['function_ref_id'], 'ref_id' =>  $surat_id, 'agenda' => ('SK -' . $_POST['agenda_id']), 'note' => $note, 'detail_link' => ('surat/keputusan/keputusan_view/' . $surat_id), 'notify_user_id' => get_user_id(), 'read' => 0));
