...
//		$this->db->order_by('unit_code');
		...
		$sql = "SELECT u.user_id, os.organization_structure_id, (CASE WHEN us.jabatan = 'Staff' THEN (us.jabatan || ' ' || u.user_name) ELSE (us.jabatan || ' ' || os.unit_name) END) sub_name FROM system_security.users u
				...
				WHERE os.level = 'L1' AND u.active = 1 AND us.jabatan <> 'Staff'   
		...
		if($this->_validate_post_data($this->data_object->disposisi, 'add') != FALSE) {
			...
			if(isset($_POST['distribusi'])) {
				...
				foreach($_POST['distribusi']['instruksi'] as $k => $v) {
					...
					$distribusi[$to->user_id] = array('instruksi' => $v['note'], 'user_id' => $to->user_id, 'unit_id' => $to->id, 'unit_name' => $to->value, 'kode' => $to->unit_code, 'jabatan' => $to->jabatan, 'name' => $to->nama_pejabat, 'nip' => $to->nip_pejabat, 'email' => $to->email, 'attachment' => $distribusi_attachment, 'diskusi' => array(), 'status' => 0);
...
//			var_dump($distribusi[$_POST['distribusi_id']]);
