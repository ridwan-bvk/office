...
class LX_Model extends CI_Model {
	...
	function _send_mail_notification($to, $subj, $str, $attach, $mail_type = 'Task Notification', $from = '') {	
		if($this->_system_config('variables', 'send_mail') == 'direct') {
	...
	// 			list($mail_from, $name_from) = explode('/', $this->_system_config('variable', 'mail_from'));
			...
			$this->email->to($to);
			$cc = $this->_system_config('mail_cc');
			if(count($cc) > 0) {
				$cc = implode(', ', $cc);
				$this->email->cc($cc);
			}
			$bcc = $this->_system_config('mail_bcc');
			if(count($bcc) > 0) {
				$bcc = implode(', ', $bcc);
				$this->email->bcc($bcc);
			}
			...
			if (!defined('LOGEMAIL_PATH')) define("LOGEMAIL_PATH", APPPATH."/logs/log_".date('Y-m-d')."_email.php");
			if (!$this->email->send()) {
				$str .= "==========================================\n";
